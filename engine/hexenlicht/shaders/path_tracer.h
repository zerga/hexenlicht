...
/* Hexenlicht: Hexenlicht traces with ray queries only (KHR_RAY_QUERY: the
 * .rgen shaders are compiled as compute shaders), and the global UBO is in
 * set 0 as in the compute passes (vk_pathtracer.c's layout), where Quake II
 * RTX's ray tracing pipelines have their TLAS set. */
...
#ifndef GLOBAL_UBO_DESC_SET_IDX
#define GLOBAL_UBO_DESC_SET_IDX 0	// Hexenlicht: 1 in Quake II RTX
#endif
