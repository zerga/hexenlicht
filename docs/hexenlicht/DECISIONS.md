...
| R15 | No images only for DLSS Ray Reconstruction: its inputs come from the G-buffer (RENDERER.md, 3D view); the specular hit distance comes with the reflections (3.5); *the one exception: R28*; *revised by R54: RR needs screen-layout images computed from the G-buffer* | 3.2 |
| R16 | Q2RTX's real-time settings for primary rays: no depth of field (`pt_aperture` 0; Q2RTX only uses it when accumulating), no jitter until TAA (3.8); *jitter with TAAU and FSR since 3.8 (R44, R45)* | 3.2 |
...
| R28 | The first bounce stores the specular ray's hit distance for DLSS Ray Reconstruction (`PT_SPECULAR_HIT_DIST`, r16f, 0 without a specular ray: a diffuse bounce, no surface, lava, the rows half resolution skips); 3.9 decides whether RR needs more (*answered: R55*) | 3.5a |
...
| R34 | The denoiser is Q2RTX's A-SVGF (`asvgf_*.comp`, `vk_asvgf.c`) without its TAA (3.8) (*since 3.8 the upscaler's, R44*), on by default with Q2RTX's `flt_*` defaults; `flt_enable 0` is the undenoised composite, as before 3.6 | 3.6 (#36) |
...
| R44 | One upscaler interface (`vk_upscale.c`: Q2RTX's `get_render_extent`, `evaluate_taa_settings`, TAA and FSR 1) decides per frame the render size, jitter, TAA mode and output and FSR; the TAA pass (`VK_UpscaleHDR`) runs before bloom and tone mapping, FSR (`VK_UpscaleDisplay`) after them; DLSS (3.10) takes the TAA pass's place | 3.8 (#38) |
| R45 | `r_upscaler` 0 TAA (render size, no jitter), 1 TAAU (default: jittered, upsampled to the view), 2 FSR 1 (jittered TAA at the render size, then EASU + RCAS; only below 100 % and with tone mapping, else TAAU); Q2RTX's `flt_taa` follows it; TAA only for the lit image and with the denoiser (Q2RTX's rule), a copy without history | 3.8 |
| R46 | Our TAA keeps primary rays at the pixel centers: Q2RTX's `AA_MODE_TAA` moves each to a random point in its pixel; every mode gives the UBO `AA_MODE_UPSCALE` and the jitter and output size decide | 3.8 |
| R47 | `r_scale` 25–100 % (not Q2RTX's `scr_viewsize`: Hexen II's `viewsize` is the HUD layout); the render targets keep the swapchain's size, no new images on a change; the upscalers' output is the view with its width rounded up to even, shown 1:1; no dynamic resolution (needs GPU timers: 3.11 or 7.2) and no scales above 100 % | 3.8 |
...
| R52 | DLSS SR and RR run through Streamline on our Vulkan device (spike 3.9: Streamline 2.14.1, NGX 310.9.1): `sl.interposer.dll` is loaded at runtime when present, `slInit` runs before the instance (engine "custom", a version and our project GUID, no NVIDIA application ID), and volk loads Vulkan through the interposer's `vkGetInstanceProcAddr`, whose instance/device proxies add DLSS's extensions; the device also enables Vulkan 1.3 `privateData`. Streamline's default preference flags turn on over-the-air updates and downloaded plugins: we set only `eDisableCLStateTracking` (every later pass binds its own pipeline, sets and push constants) and `eUseFrameBasedResourceTagging`. Streamline's headers are C++: one C++ file with a C interface. Without the DLLs nothing changes (TAAU instead) | 3.9 (#39) |
| R53 | DLSS's conventions against ours: its jitter offset is the negation of our `sub_pixel_jitter` (measured: the wrong sign blurs a still frame and makes it flicker); our motion vectors (previous minus current position, UV units) go in with `mvecScale` (1, 1); our column-major matrices are Streamline's row-major ones unchanged; Quake II RTX's projection has no usable depth row, so DLSS gets a D3D-style one (0 near, 1 far) and hardware depth computed from the view depth, which is the G-buffer's distance along the ray (`PT_VIEW_DEPTH`) divided by the length of the pixel's view-space direction at z = 1; `preExposure` 1 and 128 look the same (our ×128 storage scale can stay) | 3.9 |
| R54 | RR gets the noisy lit image (no denoiser, and every specular ray counts: Quake II RTX's `pt_fake_roughness_threshold` 1 without the denoiser) and its inputs in the screen layout at the render size (diffuse and specular albedo, normal + roughness, depth, specular hit distance): new images, computed from the G-buffer's fields | 3.9 |
| R55 | RR accepts our specular hit distance with its zeros (no specular ray) and needs no value in every pixel: on demo1's floor made a metal mirror (roughness 0.02) or glossy metal (0.15) RR's reflections stay crisp during turns, where A-SVGF + TAAU's turn grainy or vanish, and zeroing the hit distance changes nothing visible; revisit with E5's materials | 3.9 |
| R56 | RR does not resolve our checkerboard fields: at a translucent surface (the cathedral's font) it keeps the per-pixel alternation where A-SVGF + TAAU blends it (most likely by the interleave's blur: from another view TAAU at 67 % keeps the pattern too, 3.12), so RR's input must be resolved first (3.10); resolving it for RR is untested, as no view was found where the blur triggers for RR's input | 3.9 |
| R57 | Measured (test lights, `tga_mean.ps1`'s noise and 60-pixel blocks): at 100 % RR (as DLAA) is closer to a low-noise reference (32 raw frames averaged, direct light, linear) than A-SVGF + TAA (worst block 5.2 % off against 12.1 %); paused at 67 % RR is steadier than A-SVGF + TAAU (temporal noise 0.019 against 0.029). It costs more (2560x1440, the GPU at a ~100 W power cap): RR with its input pass 8.1 ms at 67 % against A-SVGF + TAAU's 2.8 ms (the whole view 12.4 against 6.7 ms); SR (with the same input pass, of which it needs only the depth) 2.5 ms against the TAA pass's 0.37. A-SVGF + TAAU stays the default, DLSS an option (RR's default preset F) | 3.9 |

## Open questions carried forward

See [Q2RTX.md](Q2RTX.md#open-questions-for-later-stories): water normal map
and physical water (6.5), checkerboard fields and RR (3.10, 3.12), specular hit distance (E5),
...
(E5, 4.5), dark albedo (E4, E5), dynamic resolution (3.11, 7.2), hot spots
and the TAA's PQ clamp (4.9), reference shots by accumulation (4.9).
